...
13. [Billing & Credits System](#13-billing--credits-system)
...
- **Full VS Code-grade editing experience** via the Monaco Editor.
...
- **Edge-native backend** (Cloudflare Workers + Hono + R2 + D1 + KV + Durable Objects) for global low latency, cost efficiency, and strong consistency where needed.
- **Multi-layer caching strategy** (provider, edge, browser, dedup) to reduce latency and AI costs.
- **Credits-based billing model** with a generous free tier and a Pro upgrade.
...
- **Model-agnostic beats single-model lock-in.**
- **Edge-native infrastructure delivers global speed at low marginal cost.**
- **Preview + edit + history turns one-shot generation into a real iterative workflow.**
- **Credits align cost with value and protect margins.**
- **Intelligent caching at multiple layers minimizes latency and AI spend.**
...
- Model catalog served from edge cache (TTL: 5 minutes).
...
- Project list responses cached at edge (TTL: 30 seconds, stale-while-revalidate: 2 minutes).
...
└───────┬───────────────┬───────────────┬───────────────┬────────────┘
 ...
 │ Caching    │  │ for file     │ │ user_id      │   ┌──────────┐
 └────────────┘  │ blobs        │ │ (hash-based) │   │  Clerk   │
                 └──────────────┘ └──────────────┘   │ (auth +  │
   ┌──────────────┐                                  │ webhooks)│
   │ Cloudflare KV│                                  └──────────┘
   │ (model-      │
...
| **CreditMeter Durable Object** | Strongly-consistent credit reserve/debit/refund; concurrency slot enforcement; per-user rate limiting. The live counter + lock. |
...
| **D1 (Sharded)** | Source of truth for users, projects, versions, and the credit-transaction ledger. Sharded by a hash of user_id. |
...
Client          Worker (Hono)        KV/Cache    CreditMeter DO     AI Provider     R2 / D1(Shard) / Queue
  ...
  │                │◀── miss ──────────│              │                  │                  │
  │                │ resolveD1Shard    │              │                  │                  │
  │                │ reserve(estCreds) ─────────────▶│ serialized check │                  │
  ...
  │◀───────────────│ pipe via Transform              │                  │                  │
  │ ...keep-alive...│ (ping every ~15–25s)           │                  │                  │
  │                │ on complete:    │              │                  │                  │
  │                │  parse files    │              │                  │                  │
  │                │  write blobs ───┼──────────────┼──────────────────┼─────────────────▶│ R2
  │                │  insert version ┼──────────────┼──────────────────┼─────────────────▶│ D1
  │                │  settle(actualC)─────────────▶│ debit + release  │                  │
  │                │  save dedup ───▶│ KV (2 min)  │                  │                  │
  │                │  enqueue tasks ┼──────────────┼──────────────────┼─────────────────▶│ Queue
  │                │  invalidate list│              │                  │                  │
  │ SSE done(vid)   │                              │                  │                  │
  │◀───────────────│                               │                  │                  │
  │ (on abort) ─────▶ refund(unused) ──────────────▶│ refund + release │                  │
```

**Key properties:**
- Prompt dedup check via KV prevents redundant generations.
- Credit operations serialized in DO for strong consistency.
- Critical version row + manifest written inline (versionId returns immediately).
- Non-critical artifacts produced asynchronously via Queues.
- Project list edge cache invalidated on mutation.
...
| Relational store | Cloudflare D1 (Sharded) — shard key is hash of user_id |
| Strong-consistency primitives | Durable Objects — CreditMeter per user |
| Edge cache | Cloudflare Workers Cache API |
| Async jobs | Cloudflare Queues (thumbnail render, zip pre-build) |
| KV / cache | Cloudflare KV — model-catalog, config, prompt dedup |
...
- Streaming is I/O-bound; active CPU stays low. Default 30s is ample and protects against runaway/buggy generations.
- Paid default is already 10,000. Raise only if a single generation fans out to many subrequests (it normally won't).
...
|-------|----------|------|---------|----------|---|
| Claude Opus | Anthropic | Pro | | Best reasoning & code quality | High |
| Claude Sonnet | Anthropic | Free+Pro | ✓ | Balanced quality/speed | Medium |
| Claude Haiku | Anthropic | Free | | Fast, cheap | Low |
| GPT-4o | OpenAI | Pro | | Strong general + multimodal | High |
| GPT-4o mini | OpenAI | Free | | Fast, cheap | Low |
| Gemini 1.5 Pro | Google | Pro | | Long context | Medium-High |
| Gemini 1.5 Flash | Google | Free | | Fast, long context | Low |
| DeepSeek-V3 / Coder | DeepSeek | Free+Pro | | Code-specialized, cost-efficient | Low |
...
Body: {
  ...
  "context": {                   // optional existing files for iteration
    "files": [...]
  }
}
...
Response: { "downloadUrl": "string" }   // signed R2 URL, short TTL
```
...
> **Note:** Pricing and credit allotments are placeholders pending finance modeling (see Open Questions).

### 13.3 Credit Lifecycle

**Signup grant:** free credits written to D1 ledger + DO state via Clerk webhook.
...
**Plan refill:** monthly refill on the billing cycle (Stripe webhook) updates the user's D1 shard ledger + DO state.

**Top-ups (P1):** one-time credit purchases.
...
- **Metrics:**
  ...
  - credit debit accuracy
  - D1 shard size/usage
  - cache-related latency reduction
...
- **Requests:** 10M included (+$0.30 / additional million).
- **CPU-ms:** 30M included (+$0.02 / additional million).
- **Duration:** No charge or limit (wall-clock). CPU time only counts active execution.
...
- **Storage:** $0.75/GB/mo.
- **Reads:** 25B rows/month free, then $0.001/million rows.
- **Writes:** 50M rows/month free, then $1.00/million rows.
