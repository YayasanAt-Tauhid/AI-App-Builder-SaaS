/**
 * schema.ts — SQLite DDL for one D1 shard (PRD Section 11.1).
 *
 * Each shard carries the full schema; a user's rows all live in their shard.
 * Plain SQLite types keep this portable to Cloudflare D1 verbatim. Run on
 * every shard open (CREATE TABLE IF NOT EXISTS makes it idempotent).
 */
