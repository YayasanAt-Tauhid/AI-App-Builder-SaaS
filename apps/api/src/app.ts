/**
 * app.ts — The Hono application (no runtime side effects).
 *
 * Builds and exports the `app` shared by both entrypoints: the Node server
 * (index.ts) and the Cloudflare Worker (worker.ts). It must NOT install a
 * backend or start a server — each entrypoint does that for its runtime — so
